...
class Appointment {
  ...
  static async getAllByUserAndStatusAndPet(userId, status, petId) {
    const query = { userId, petId };

    if (status) {
      query.status = status;
    }
    return AppointmentModel.find(query).sort({ date: 1 });
  }
  static async getAllByUserAndPet(userId, petId) {
    return AppointmentModel.find({ userId, petId }).sort({ date: 1 });
  }
